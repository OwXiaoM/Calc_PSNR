...
# psnr = calc_psnr(hr_im, lr_im, data_range = R)
# ssim = calc_ssim(hr_im, lr_im, gaussian_weights = True, data_range = R, multichannel=False,
#                         use_sample_covariance = False, pad = 0)
#

# from this file import *, then the item listed in __all__ will be import
__all__ = ['calc_psnr', 'calc_ssim']


def calc_psnr(X, Y, data_range=None):
...
def calc_ssim(X, Y, win_size=None, gradient=False, data_range=None,
    ...
    if multichannel:
        ...
        if full:
            S = np.empty(X.shape)
        for ch in range(nch):
            ch_result = calc_ssim(X[..., ch], Y[..., ch], **args)
